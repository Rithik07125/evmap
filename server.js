...
// Load the map page once at startup (cheap, small file, no need to re-read per request)
const MAP_HTML_PATH = path.join(__dirname, 'public_map.html');
let mapHtml = null;
try {
  mapHtml = fs.readFileSync(MAP_HTML_PATH, 'utf8');
} catch (e) {
  console.error('Could not load public_map.html — the / route will 404 until this file exists next to server.js');
}
...
http.createServer(async (req, res) => {
  ...
  } else if (req.url === '/' || req.url === '/map') {
    // Serve the public map page at the root URL
    if (mapHtml) {
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      res.end(mapHtml);
    } else {
      res.writeHead(500, { 'Content-Type': 'text/plain' });
      res.end('Map page not found on server — public_map.html is missing.');
    }
